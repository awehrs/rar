...
class Docstore:
    ...
    def get_nearest_examples(
        ...
        Args:
            ...
            metadata_field: The metadata field in the huggingface dataset to return: either
        ...
        total_samples_filtered = [
            {key: sample[key] for key in self.time_index + [metadata_field]}
            for sample in total_samples
        ]
        return total_scores, total_samples_filtered
    ...
    def date_aligned_knn(
        ...
        Args:
            ...
            k_neighbors: Number of results to return per query.
            ...
            metadata_field: The metadata field in the huggingface dataset to return (e.g.,
                "Tokens" or "Embeddings")
            pad_val: Value to pad with when there aren't enough results with sufficient
                observations.
        Returns:
            Dictionary with the following key/value pairs:
                ...
                "null_indices": tensor of size [batch_size * num_queries].
        """
        # Retrieve KNNs.
        ...
        meta = [pd.DataFrame(df[metadata_field].to_list()) for df in sample_dfs]
        ...
        # Restructure / pad.
        return self.restructure_date_aligned(
            data, meta, valid_indices, k_neighbors, num_queries, num_samples, pad_val
        )
    ...
    def resample_data(
        ...
        valid_indices = [
            _df["sufficient"] for _df in np.split(dates_df, batch_size, axis=0)
        ]
        return data_dfs, valid_indices

    def restructure_date_aligned(
        self,
        data: List[pd.DataFrame],
        meta: List[pd.DataFrame],
        valid_indices: List[pd.Series],
        k_neighbors: int,
        num_queries: int,
        num_samples: int,
        pad_val: float,
        ...
        """
        Restructure data for consumption by date_aligned_knn. Filter invalid rows in data
            and metadata. Replace with padding vectors.

        Args:
            data: List of dataframes of length [batch_size * num_queries].
                Each frame contains data for K nearest neighbors of query.
            meta: List of dataframes of length [batch_size * num_queries].
                Each frame contains metadata for K nearest neighbors of query.
            valid_indices: List of pandas series of length = [batch_size * num_neighbors].
                Each series is boolean index of valid rows in corresponding data/metadata
                dataframe.
            k_neighbors: Number of results to return per query.
            num_queries: Number of queries generated by by query encoder.
            num_samples: The number of data points to return.
        ...
        Returns:
            Dictionary with the following key/value pairs:
                "metadata": tensor of size [batch_size, num_queries, k_neighbors, emb_dim],
                "data": tensor of size [batch_size, num_queries, k_neighbors, num_samples],
                "mask": tensor of size [batch_size, num_queries, k_neighbors].
        """
        data_tensors, meta_tensors, null_indices = [], [], []

        for pos in range(len(data)):
            # Filter insufficiently covered series.
            data_tensor = torch.tensor(
                data[pos][valid_indices[pos] == True].values, 
                dtype=torch.float32
            )
            meta_tensor = torch.tensor(
                meta[pos][valid_indices[pos] == True].values,
                dtype=torch.float32,
            )
            if data_tensor.nelement() == 0:
                # No neighbors with sufficient coverage.
                data_tensors.append(torch.full((k_neighbors, num_samples), pad_val))
                meta_tensors.append(torch.full((k_neighbors, num_samples), pad_val))
                null_indices.append(torch.ones(k_neighbors))

            elif data_tensor.shape[0] < k_neighbors:
                # Less than K valid neighbors.
                ...
                null_indices.append(
                    torch.cat(
                        [
                            torch.zeros(k_neighbors - pad_len),
                            torch.ones(pad_len),
                        ],
                        axis=0,
                    )
                ...
                # Sufficient valid neighbors.
                data_tensors.append(data_tensor[:k_neighbors, :])
                meta_tensors.append(meta_tensor[:k_neighbors, :])
                null_indices.append(torch.zeros(k_neighbors))

        data = rearrange(data_tensors, "(b n) k d -> b n k d", n=num_queries)
        meta = rearrange(meta_tensors, "(b n) k d -> b n k d", n=num_queries)
        mask = rearrange(null_indices, "(b n) k -> b n k ", n=num_queries)

        return {"data": data, "metadata": meta, "mask": mask}
